...
var _ = Describe("BatchRecordReader", func() {
	When("successfully", func() {
		...
		It("default batch", func() {
			...
			brr := NewBatchRecordReader(rr, "none", WithBatch(0))
		...
		It("1 batch", func() {
			...
			brr := NewBatchRecordReader(rr, "none", WithBatch(1))
			nBytes, err = brr.Size()
			Expect(err).NotTo(HaveOccurred())
			Expect(nBytes).To(Equal(int64(33)))
		...
		It("2 batch", func() {
			...
			brr := NewBatchRecordReader(rr, "none", WithBatch(1), WithBatch(2))
			nBytes, err = brr.Size()
			Expect(err).NotTo(HaveOccurred())
			Expect(nBytes).To(Equal(int64(33)))
		...
		It("3 batch", func() {
			...
			brr := NewBatchRecordReader(rr, "none", WithBatch(1), WithBatch(3))
			nBytes, err = brr.Size()
			Expect(err).NotTo(HaveOccurred())
			Expect(nBytes).To(Equal(int64(33)))
		...
		It("4 batch", func() {
			...
			brr := NewBatchRecordReader(rr, "none", WithBatch(1), WithBatch(4))
			nBytes, err = brr.Size()
			Expect(err).NotTo(HaveOccurred())
			Expect(nBytes).To(Equal(int64(33)))
